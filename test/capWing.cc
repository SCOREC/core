...
int main(int argc, char** argv)
{
  ...
  ma::AnisotropicFunction* sf = new WingShock(apfCapMesh, 50);
  ...
  long numElem = countElements(apfCapMesh), numElemRef = 13016;
  PCU_ALWAYS_ASSERT(std::abs(numElem - numElemRef) < 0.05*numElemRef);

  long numVert = apfCapMesh->count(0), numVertRef = 2171;
