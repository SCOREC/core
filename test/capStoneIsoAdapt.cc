...
int main(int argc, char** argv)
{
  ...
  if (argc < 4) {
    if (PCU_Comm_Self() == 0) {
      printf("USAGE: %s <in_cre_file.cre> <out_cre_file.crc> <factor>\n", argv[0]);
    }
    MPI_Finalize();
    exit(EXIT_FAILURE);
  }
  ...
  ma::Input* in;
  ma::IsotropicFunction* sf = new Geom(apfCapMesh, factor);
  ...
  ma::visualizeGeometricInfo(apfCapMesh, "geom");
