...
static void for_type_net(PCUHandle h, struct mds_net* net, struct mds* m,
    int t, void (*f)(PCUHandle h, mds_id i, struct mds_copy c, void* u), void* u)
{
  mds_id i;
  int j;
  struct mds_copies* cs;
...
static void note_remote_link(PCUHandle h, mds_id i, struct mds_copy c, void* u)
{
  (void)i;
  if (c.p != PCU_Comm_Self2(h))
    note_peer(u, c.p);
}
...
static void take_remote_link(PCUHandle h, mds_id i, struct mds_copy c, void* u)
{
  struct mds_links* ln = u;
  int pi;
  if (PCU_Comm_Self2(h) < c.p) {
    pi = find_peer(ln, c.p);
    ln->l[pi][ln->n[pi]] = i;
...
static void send_remote_link(PCUHandle h, mds_id i, struct mds_copy c, void* u)
{
  (void)i;
  (void)u;
  if (PCU_Comm_Self2(h) < c.p)
...
static void recv_links(PCUHandle h, struct mds_links* ln)
{
  int from;
  mds_id* tmp;
  int pi;
  unsigned i;
  from = PCU_Comm_Sender2(h);
  pi = find_peer(ln, from);
  tmp = PCU_Comm_Extract2(h, ln->n[pi] * sizeof(mds_id));
  for (i = 0; i < ln->n[pi]; ++i)
    ...
    int t, struct mds_links* ln)
{
  ...
  alloc_links(ln);
  for (i = 0; i < ln->np; ++i)
    if (((unsigned)PCU_Comm_Self2(h)) < ln->p[i])
    ...
    int t, struct mds_links* ln)
{
  ...
  for (i = 0; i < ln->np; ++i) {
    ...
    for (j = 0; j < ln->n[i]; ++j)
      PCU_COMM_PACK2(h, ln->p[i], ln->l[i][j]);
  }
  PCU_Comm_Send2(h);
  while (PCU_Comm_Listen2(h)) {
    c.p = PCU_Comm_Sender2(h);
    PCU_ALWAYS_ASSERT(c.p != PCU_Comm_Self2(h));
    i = find_peer(ln, c.p);
    in = PCU_Comm_Extract2(h, ln->n[i] * sizeof(unsigned));
    for (j = 0; j < ln->n[i]; ++j) {
...
static void note_local_link(PCUHandle h, mds_id i, struct mds_copy c, void* u)
{
  if (c.p == PCU_Comm_Self2(h)) {
    if (i < mds_index(c.e))
      note_peer(u, PCU_Comm_Self2(h));
    else /* hack id to store self-receivers */
      note_peer(u, PCU_Comm_Peers2(h));
  }
}

static void take_local_link(PCUHandle h, mds_id i, struct mds_copy c, void* u)
{
  struct mds_links* ln = u;
  int self = find_peer(ln, PCU_Comm_Self2(h));
  int other = find_peer(ln, PCU_Comm_Peers2(h));
  mds_id j = mds_index(c.e);
  if ((PCU_Comm_Self2(h) == c.p) && (i < j)) {
                         ...
                         int t, struct mds_links* ln)
{
  ...
  if (self == -1)
    return;
  other = find_peer(ln, PCU_Comm_Peers2(h));
