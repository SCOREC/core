...
// pp is the pole location and does lay on one of the limits of the degenerate axis
// previous coded was by considering pp = -pi/2 or pi/2 (not zero)
// have it converted to account for pp=0
// for strictly increasing parametric coordinates, i.e. theta > 0 over the surface
// pp is corresponding to the south pole as previously considered =
  //double d_range[2];
  //m->getPeriodicRange(g, d_axes, d_range[2]);
  d_size = std::abs(d_range[0]-d_range[1]);
// d_size = std::abs(2.0 * pp);   

  p_threshold = p_ratio * p_size;
  d_threshold = d_ratio * d_size;
  
  double nsp_threshold=0.1*d_threshold;	//inserted

  //check pp exist at which limit  min or max;(if min==sp; max==np;)
  ...
  // check the south pole (i.e., at least one point inside south pole space)
  // if (pp<0){
  if (std::abs(pp-d_range[0]) <=nsp_threshold) {	//inserted
   //printf (".........near south pole....");
    
    double sp = pp;
    
    if (std::abs(the_1 - sp) <= nsp_threshold) {
      if (std::abs(the_2 - sp) <= nsp_threshold) {
	if (std::abs(phi_span - p_size/2.) > p_threshold) {
	  phi_t = phi_tmp;
	  flag = true;
	}
  ...
  // check the north pole (i.e., at lease one point inside north pole space)
  //if (pp > 0) {
  if (std::abs(pp-d_range[1]) <= nsp_threshold) {	//inserted
   //printf (".........near north pole....");
    double np = d_range[1];	//pp;
    if (std::abs(the_2 - np) <= nsp_threshold) {
      if (std::abs(the_1 - np) <= nsp_threshold) {
	if (std::abs(phi_span - p_size/2.) > p_threshold) {
	  phi_t = phi_tmp;
	  flag = true;
	}
    ...
    Vector& p)
{
  ...
  /*
  if (m->getModelTag(g) == 22) {
    printf("=========\n");
    printf("at face %d the number of poles is %d\n", m->getModelTag(g), (int)num);
    printf("=========\n");
  }
  */
