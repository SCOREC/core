...
int main(int argc, char** argv)
{
  ...
  double minArea = addSizeDistribution(mesh, factor, binCount, binArea);

  printf("min area is %f\n", minArea);

  int a_lower_factor = 1;
  for (std::size_t i = 0; i < binCount.size(); i++) {
    double mn = i == 0 ? 0 : a_lower_factor * minArea;
    double mx = i == 0 ? minArea : 4 * a_lower_factor * minArea;
    printf("bin %lu's [%f,%f] count/area is %d/%g \n", i, mn, mx, binCount[i], binArea[i]);
    a_lower_factor *= i == 0 ? 1 : 4;
  }

  double totalBinArea = 0.0;
  int totalBinCount   = 0;

  for (std::size_t i = 0; i < binCount.size(); i++) {
    totalBinCount += binCount[i];
    totalBinArea  += binArea[i];
  }

  printf("total     area is %E \n", totalArea(mesh));
  printf("total bin area is %E \n", totalBinArea);
